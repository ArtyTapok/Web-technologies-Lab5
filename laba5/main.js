...
function downloadData(page = 1, searchRequest = "") {
    let factsList = document.querySelector('.facts-list');
    ...
    if (searchRequest != "") {
        url.searchParams.append('q', searchRequest);
    }
...
function showAutocompletes(suggestionList) {
    ...
    if (suggestionList.length > 0) {
        autoCompletesBox.classList.remove("hide");
        for (let suggestion of suggestionList) {
            ...
            li.onclick = function () {
                ...
                autoCompletesBox.classList.add("hide");
            };
...
function getAutocompletes(event) {
    let url =
    ...
    xhr.responseType = 'json';
    xhr.onload = function () {
        showAutocompletes(this.response);
    };
    xhr.send();
}
...
window.onload = function () {
    ...
    document.querySelector(".search-field").oninput = getAutocompletes;
    document.querySelector(".search-btn").onclick = searchFacts;
};